...
def espectaculoZoologico(n, m, k, animales, grandezas, apertura, partes):
    ...
    def ordenarEscenaPorGrandeza(escenas):
        sorted_escenas = []
        for escena in escenas:
            count = [0] * (max(grandezas) + 1)
            for animal in escena:
    ...
    promedio_grandezas = suma_total_grandezas / sum(len(escena) for parte in [apertura] + partes for escena in parte)
...
def medir_tiempo_espectaculo_zoologico(n, m, k, animales, grandezas, apertura, partes):
    inicio = time.time()
    espectaculoZoologico(n, m, k, animales, grandezas, apertura, partes)
    fin = time.time()
